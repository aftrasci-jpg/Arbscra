...
export function calculateScore(
  ...
  if (cleanWord.length === 1) {
    ...
    const singleLetterVal = FRENCH_LETTER_VALUES[singleLetter] !== undefined ? FRENCH_LETTER_VALUES[singleLetter] : 0;
    ...
    while (currC >= 0 && currentBoard[row][currC] !== "") {
      ...
      leftLetters.push({ letter: bLetter, val: FRENCH_LETTER_VALUES[bLetter] || 0 });
      currC--;
    }
    leftLetters.reverse();
    ...
    while (currC2 < 15 && currentBoard[row][currC2] !== "") {
      ...
      rightLetters.push({ letter: bLetter, val: FRENCH_LETTER_VALUES[bLetter] || 0 });
      currC2++;
    }
    ...
    if (horizLength >= 2) {
      horizWord = [...leftLetters.map(l => l.letter), singleLetter, ...rightLetters.map(l => l.letter)].join("");
    ...
    while (currR >= 0 && currentBoard[currR][col] !== "") {
      ...
      upLetters.push({ letter: bLetter, val: FRENCH_LETTER_VALUES[bLetter] || 0 });
      currR--;
    }
    upLetters.reverse();
    ...
    while (currR2 < 15 && currentBoard[currR2][col] !== "") {
      ...
      downLetters.push({ letter: bLetter, val: FRENCH_LETTER_VALUES[bLetter] || 0 });
      currR2++;
    }
    ...
    if (vertLength >= 2) {
      vertWord = [...upLetters.map(l => l.letter), singleLetter, ...downLetters.map(l => l.letter)].join("");
    ...
    let finalBreakdown = `Pose d'une seule lettre '${singleLetter}' : `;
  ...
  for (let i = 0; i < cleanWord.length; i++) {
    ...
    const value = FRENCH_LETTER_VALUES[letter] !== undefined ? FRENCH_LETTER_VALUES[letter] : 0;
  ...
  for (const tile of tilesInfo) {
    if (tile.isNew) {
      ...
      if (letterMult > 1) {
        primaryBreakdownParts.push(`${tile.letter}(${tile.value}x${letterMult}=${tileScore})`);
      } else {
        primaryBreakdownParts.push(`${tile.letter}(${tile.value})`);
      }
  ...
  for (const tile of tilesInfo) {
    ...
    if (direction === "H") {
      ...
      if (totalLength >= 2) {
        const wordStr = [...upLetters.map(l => l.letter), tile.letter, ...downLetters.map(l => l.letter)].join("");
        ...
        const parts: string[] = [];
        upLetters.forEach(l => parts.push(`${l.letter}(${l.val})`));
        if (letterMult > 1) {
          parts.push(`${tile.letter}(${tile.value}x${letterMult}=${mainTileScore})`);
        } else {
          parts.push(`${tile.letter}(${tile.value})`);
        }
        downLetters.forEach(l => parts.push(`${l.letter}(${l.val})`));
      ...
      if (totalLength >= 2) {
        const wordStr = [...leftLetters.map(l => l.letter), tile.letter, ...rightLetters.map(l => l.letter)].join("");
        ...
        // Breakdown presentation
        const parts: string[] = [];
        leftLetters.forEach(l => parts.push(`${l.letter}(${l.val})`));
        if (letterMult > 1) {
          parts.push(`${tile.letter}(${tile.value}x${letterMult}=${mainTileScore})`);
        } else {
          parts.push(`${tile.letter}(${tile.value})`);
        }
        rightLetters.forEach(l => parts.push(`${l.letter}(${l.val})`));
  ...
  let finalBreakdown = `${cleanWord} : ${primaryBreakdownStr}`;
